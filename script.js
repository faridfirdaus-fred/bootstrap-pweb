...
const initializeHomePage = () => {
  ...
  if (featuredPostElement) {
    ...
    featuredPostElement.querySelector(".badge").textContent =
      featuredPost.category.name;
    featuredPostElement.querySelector("h2").textContent = featuredPost.title;
    featuredPostElement.querySelector(".card-text").textContent =
      featuredPost.excerpt;
    featuredPostElement.querySelector("img.rounded-circle").src =
      featuredPost.author.avatar;
    featuredPostElement.querySelector("small.text-muted").textContent =
      formatDate(featuredPost.publishedAt);
    featuredPostElement.querySelector(
      "a.btn"
    ).href = `article.html?slug=${featuredPost.slug}`;
  }
  ...
  blogData.posts.slice(1, 5).forEach((post, index) => {
    if (articleCards[index]) {
      ...
      card.querySelector("small.text-muted").textContent = formatDate(
        post.publishedAt
      );
      card.querySelector(
  ...
  if (categoryList) {
    categoryList.innerHTML = blogData.categories
      .map(
        (category) => `
  ...
  if (popularPosts) {
    popularPosts.innerHTML = blogData.posts
      .slice(0, 3)
      .map(
        (post, index) => `
            <div class="d-flex mb-3">
                <img src="${post.featuredImage}" class="rounded me-3" alt="${
          post.title
        }" style="width: 80px; height: 80px; object-fit: cover;">
                <div>
                    <h6 class="mb-1">${post.title}</h6>
                    <small class="text-muted d-block mb-1">${formatDate(
                      post.publishedAt
                    )}</small>
...
const initializeArticlePage = () => {
    ...
    articleHeader.innerHTML = `
            ...
            <div class="d-flex align-items-center justify-content-center mb-4">
                ...
                <div class="text-start">
                    ...
                    <p class="text-muted small mb-0">Dipublikasikan: ${formatDate(
                      post.publishedAt
                    )} · ${post.readTime} membaca</p>
                </div>
            </div>
        `;
  }
  ...
  if (relatedPosts) {
    const relatedPostsData = blogData.posts.filter((p) =>
      post.relatedPosts.includes(p.id)
    );
    relatedPosts.innerHTML = relatedPostsData
      .map(
        (relatedPost) => `
            <div class="col">
        ...
        (comment) => `
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <button class="btn btn-sm btn-link text-decoration-none p-0 reply-btn" data-comment-id="${
                      comment.id
                    }">
...
const initializePagination = () => {
  ...
  const updatePagination = () => {
    ...
    pageLinks.forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        const isPrev = target.getAttribute("aria-label") === "Previous";
        const isNext = target.getAttribute("aria-label") === "Next";
  ...
  const displayPosts = (page) => {
    ...
    const postsToShow = blogData.posts.slice(startIndex, endIndex);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", function (e) {
      ...
      alert(
        `Terima kasih telah berlangganan newsletter dengan email: ${email}`
      );
